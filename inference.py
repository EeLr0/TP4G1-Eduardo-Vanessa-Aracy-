...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPos in self.allPositions:
            probObs = self.getObservationProb(observation, gameState.getPacmanPosition(), ghostPos, self.getJailPosition())
            self.beliefs[ghostPos] *= probObs
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for ghostPos in self.allPositions:
            newPos = self.getPositionDistribution(gameState, ghostPos)
            for nP in newPos:
                updatedBeliefs[nP] += self.beliefs[ghostPos] * newPos[nP]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        particle = self.numParticles // len(self.legalPositions)
        counter = self.numParticles % particle
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            var = self.getObservationProb(observation, pacmanPos, particle, jailPos)
            weight[particle] += var
        if weight.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for oldPart in self.particles:
            Dist = self.getPositionDistribution(gameState, oldPart)
            nextPos = Dist.sample()
            newPart.append(nextPos)
